...
- **The installer deleted a consumer's own hook wiring on every refresh.** The settings merge did `pr_hooks[event] = entries` — the framework's list replaced the consumer's outright — so any project-added matcher in an event the framework also ships was deleted. One consumer's production guard, a `PreToolUse` hook, was silently disarmed by v4.2.0, v4.4.1 and v4.5.1 in turn; `install.sh` reported success every time, and a disarmed guard looks exactly like a guard with nothing to block. Each occurrence was caught only because that project kept its own test suite and a runbook telling someone to re-wire it by hand — the wrong shape, making a security control the consumer's chore on a schedule the framework sets. Framework wiring still wins over *framework* wiring (a consumer cannot pin a stale copy of a framework hook, which is what the whole-list replace was really protecting); the split is by command path, a hook running something under `.claude/hooks/` being framework territory. Six tests against the real merge block, three red beforehand, covering preservation, framework-wins, no duplication, the mixed-matcher case, idempotence, and an event the framework does not ship.
- **Ambient `.env` discovery is gated on the layer actually being read (T077).** The preflight generator sources every discovered `.env` with `set -a`, which exports into the process environment of *every* step — so the real consumers were npm lifecycle scripts and bundler plugins, not the named steps, and an ambient root `.env` handed `JWT_SECRET`, `SERVICE_ROLE_KEY`, `POSTGRES_PASSWORD` and `VAULT_ENC_KEY` to every generated job whether or not anything asked. `discover_env_files()` takes `explicit_only`, suppressing **ambient** discovery only — an explicit `[tool.forge.preflight] env_files` override still resolves, because an operator who named a layer asked for it. The consumer's load-bearing signal was too narrow to ship as-is: it reused a flag computed only inside the Compose-rewrite branch, so a project with no Compose file, or whose workflow already carries bash-style `${NAME:-}` refs, would have silently lost a layer its CI mirror needs. Replaced with `_env_layer_is_load_bearing(jobs)`, recognising both the pre-rewrite `${{ env.NAME }}` marker and a workflow-authored ref, scoped to `env:` values rather than `run:` bodies. Five tests, including one pinning the **call site** end to end — in the consumer the parameter and the argument were reverted independently, and a unit test alone stayed green.
- **`/audit-task-status` read one counter idiom and ticked the epics it could not read.** Epic files are human-authored prose and each project settles on its own phrasing, so the lone `N total … M completed` regex matched none of one consumer's epics — every one wrote `**Status:** in_progress — 23/50 tasks completed`. And with `claimed=None`, `stale` is False, so the flag printed a green tick beside an epic nothing had checked: section [3] passed vacuously for a whole repo while E01's counter had drifted to 23/50 against a registry saying 32/56. `parse_counter_claim()` now tries four idioms, header lines first so a progress-log sentence is not mistaken for the current claim, with the whole-text fallback preserving previous behaviour; an unparseable counter renders `– … (NOT CHECKED — no counter line found; verify by hand)`. Nine tests; the rendering half runs the real CLI against a fixture project and was verified to go red when the flag line alone is reverted — a parser that matches nothing must not be indistinguishable from a clean audit.
...
- **`Bash(awk *)` is withdrawn from the shipped `allow` set.** awk executes shell (`awk 'BEGIN{system(...)}'`), writes files via `print > "f"` and pipes to `sh`, so as an *allow* rule it granted unprompted arbitrary command execution and outranked every `ask` rule beneath it — none of `rm`, `git push`, `git reset --hard` or `ssh` match a command whose first word is `awk`. One consumer removed it on 2026-08-31 for exactly this reason and the union merge put it straight back on the next refresh. It is listed under `_retired_permissions.allow` so the merge removes it from consumers instead. `sed`, `grep` and `python3` cover the text slicing it was added for; awk still runs, it just prompts like any other Bash command.
...
- The old "16 concurrent / 1000-agent" Workflow numbers were unverifiable and are **deleted, not corrected**. A Workflow's size ceiling is a per-session guideline the user sets in `/config`, not a fixed number this framework can state.
...
- **A written preservation contract for continuity artifacts** (`skills/_shared/continuity-preservation.md`). `/reflect handoff` and the `## Continuation Context` are client-side compaction: a long session is discarded and a short artifact stands in for it, and what the artifact drops is gone — the next session does not know it is missing and re-derives it, wrongly or not at all. The contract binds the six categories from the Fable 5.1 guide's compaction-summary instruction on every handoff, continuation and session-end write, and **completeness on the six outranks every brevity budget in the framework**. The handoff schema grows 5 fields → 7: `Constraints & Decisions` (stated exactly, never paraphrased) and `Specifics` (hard-to-reconstruct detail, kept verbatim) existed nowhere before, and a category with nowhere to go is a category that gets dropped. Counts are now **re-derived from their probe, never copied forward** — write `39 ready (forge task ls --ready, 2026-09-12)`, never `39 ready`; a stale count carried across a boundary is the observed defect this exists to stop.
- **`AUTONOMY.md` for `/run-epic`** — the Fable 5.1 guide's "Finish the whole task" block, scoped to the framework's one genuinely unattended skill. Without it a drain iteration ends with "Shall I apply this?" and burns with nobody there to answer. **It is deliberately quarantined**: never `CLAUDE.md`, never `rules/`, never an interactive skill, because the guide states the block also makes the model less likely to ask about genuinely ambiguous requests — correct for a drain, wrong for pair work. The framework's escalation gates, hard floors and rate limiters are written into the block as its stop list, so it constrains the loop rather than loosening it. The guide's "Exception" paragraph is omitted on purpose, and the reason is recorded in the file: a `/run-epic` task *is* the standing request for the change, so a rule to report-and-stop on reading a problem description would stall every `/fix-bug` task in the epic.
- **A spawn budget for `/run-epic --parallel`** (GUARDRAILS.md §5). `--max-agents` caps a *batch*, never a *run*: the defaults alone project `3 × 50 = 150` spawns for one invocation, and nothing counted them, so a run that exhausted an environment's subagent budget discovered it mid-epic. The loop now projects and reports the number at init, clamps `--max-agents` to the environment's real concurrency cap, counts spawns across the run, and **degrades to serial rather than spawning into a refusal**.
- **"Don't duplicate the harness"** (`CONTRIBUTING.md`) — a table of what Claude Code already injects (scope discipline, the corrections limiter, subagent caps, the parallel-tool-call nudge) and must not be restated, plus the audit grep for vetting an incoming plugin or skill pack written for an older model. Re-stating an instruction the harness already gives makes behaviour worse, not better; the Opus 5 guide is explicit that such instructions compound and add cost without improving results.

### Changed

- **Routing is now a pair — model *and* effort — not a model alone** (`skills/_shared/model-routing.md`). Both guides put effort first as the primary cost/latency control, and a bigger model at low effort is frequently cheaper *and* better than a smaller model at default effort — the trade a model-only table cannot express. Dispatches now name tier, model and effort (`E2 → sonnet @ medium`). **Effort has no per-dispatch argument**: it comes from an agent definition's `effort:` frontmatter, so routing a tier below the session's effort requires a named agent rather than an ad-hoc `general-purpose` call — verified against the subagents and SDK references, and stated in the file so nobody writes an effort argument that is silently nothing. Hard floors now bind both halves: a floor task never goes to a down-routed agent definition.
- **The fan-out ceiling is documented as the environment's, not the framework's.** `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` (default `3`), `CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS` (default `20`), and the SDK's `maxBudgetUsd` query option, each with its behaviour at the limit. Two caveats that change how you bound a run: an **`ultracode` session is never refused by the concurrency cap**, so the fan-out most worth bounding is the one that env var does not bound — use the Workflow size guideline and spend instead; and a per-session total cap is **not** in the documented set, so it is honoured where an environment sets it but no default is documented and no guardrail depends on it existing.
- **Written-document length calibration** (`skills/_shared/report-format.md`, carried by every `templates/` file that authors a document). A feedback level caps a *report* and says nothing about files written to disk, which current models make longer. The rule cuts **both ways**: a document trimmed below its substance fails it exactly as a padded one does, and fails harder where content is load-bearing — the six preservation categories, an ISA's verification evidence, a task's acceptance criteria.
- **The routing table's sweep is the consuming project's job**, and says so. A route is only as good as the task shapes it was measured on, so a sweep run in the framework repo does not transfer to a consumer at all. The table ships as a defensible default every project inherits; the "Sweep against" column names the challenger (Fable at `low`, per its guide's instruction to include it wherever you would otherwise run a smaller model at a higher effort level); results land in that project's `model-routing.local.md`, which overrides the row without waiting on a framework release.
## [v4.5.0] — 2026-09-05
...
- **`/intent`** — the non-engineer entry point to the artifact chain. An intent is **change-sized, not epic-sized**: promote links it to the task ids it filed (`promoted → T###, T### (E0X)`), and links an epic only when it created that epic — first consumer run filed a one-tab bug into a 35-task epic and the original `promoted → E0X` wording claimed far more than it described. `/intent "<problem>"` brainstorms into a committed `intent/<slug>.md` (problem, outcome, affected users, constraints, open questions; template at `templates/intent.md`). `/intent promote` turns an *accepted* intent into a forge epic + tasks through the CLI and stamps the intent `promoted → E0X`. Capture and promote are separate on purpose: acceptance is a status edit by someone with authority, not a merge.
...
- **Every session started blind in consumers upgraded from v2.** `docs/project-memory/index.md` was still the v2 compiler's output (a Summary table reading 0/0/0) beside real entries — 56 in one consumer, 4 in another. v4 removed the compiler and never replaced it; the installer only created a *missing* index, so the fossil survived every refresh; and the SessionStart warning told you to run `/remember`, which has no rebuild. Now: **`forge memory reindex`** regenerates the index deterministically from the entry headings (both v2 `**Date:**` and v4 `- **Date:**` shapes, v2 table-of-contents heading skipped, newest first, idempotent); `install.sh` runs it on every refresh that keeps existing memory; the hook warning names the real command; `/remember` step 5 calls it instead of hand-editing. Windows `install.ps1` not updated — run the command manually there.
