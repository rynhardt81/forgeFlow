...
> Minor. Three patches a consumer had been re-applying by hand after every refresh, ported upstream. Two are framework code it could never have kept; the third is the installer behaviour that kept eating them. The pattern is the release: a project cannot own a fix to framework code, and a security control that has to be re-applied by hand is a security control that is off.
