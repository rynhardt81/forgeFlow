...
- Rules in `.claude/rules/` are binding. Those without `paths:` frontmatter load at session start; seven advisory ones load only when Claude **Reads** a matching file — `testing.md`, `release-engineering.md`, `hooks.md`, `error-handling.md`, `observability.md`, `patterns.md`, `coding-style.md`. When the work is in one of those domains and you reached the files through Bash rather than Read, read the rule directly. The domain ones govern specific work: schema → `migrations.md`, shipping → `release-engineering.md`, personal data → `privacy.md`, **adding or updating any dependency → `dependencies.md`** (exact pins; refuse versions published in the last 7 days — supply-chain compromises are usually caught within that window; new dependencies require explicit justification before install).
