...
- **If the root cause traces to agent behaviour** — a gate that should have fired and did not, a skill that let something through — add an eval: a case in `tests/evals/cases/` whose prompt is the situation that failed and whose check names the gate that was missed. `tests/evals/README.md` has the format. This is the only mechanism that catches the same config regression next time; a postmortem paragraph does not.
