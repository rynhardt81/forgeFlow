...
class TestGeneratedHeaderIsMachineIndependent(unittest.TestCase):
    """The header must not depend on how the caller spelled --project-root.

    `Job.file` carries whatever the glob produced. The pre-push hook passes an
    absolute `git rev-parse --show-toplevel`; the skill documents
    `--project-root .`. Left alone, the same repo produced a different header by
    each route — an absolute home path one way — so every alternation was a
    spurious diff for projects that commit `.forge/preflight/*.sh`.
    """
    ...
    def _header(self, project_root: Path) -> str:
        out = self.root / ".forge" / "preflight"
        shutil.rmtree(out, ignore_errors=True)
        run_preflight(project_root, out_dir=out)
        body = (out / "lint.sh").read_text()
        return next(ln for ln in body.splitlines() if "Generated by" in ln)

    def test_absolute_and_relative_roots_produce_the_same_header(self):
        absolute = self._header(self.root)
        cwd = Path.cwd()
        try:
            import os
            os.chdir(self.root)
            relative = self._header(Path("."))
        finally:
            os.chdir(cwd)
        self.assertEqual(absolute, relative)

    def test_header_carries_no_absolute_path(self):
        header = self._header(self.root)
        self.assertIn(".github/workflows/ci.yml", header)
        self.assertNotIn(str(self.root), header)

    def test_whole_script_carries_no_machine_path(self):
        out = self.root / ".forge" / "preflight"
        shutil.rmtree(out, ignore_errors=True)
        run_preflight(self.root, out_dir=out)
        self.assertNotIn(str(self.root), (out / "lint.sh").read_text())
